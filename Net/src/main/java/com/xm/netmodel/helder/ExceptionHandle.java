...
public class ExceptionHandle {
        ...
        if (e instanceof HttpException) {
            ...
            switch (httpException.code()) {
                ...
                case SERVICE_UNAVAILABLE:
                    ...
                    break;
                case UNAUTHORIZED:
                    L.e("handleException UNAUTHORIZED ->" + UNAUTHORIZED);
                    RequestConfig config = Config.getConfig().getRequestConfig();
                    if (config.isVerifyToken) {
                        config.loginStateListener.onStateChanged(LoginState.OUT, e.getMessage());
                        ex.message = getStringRes(R.string.unauthorized);
                    }else{
    ...
    public static String getStringRes(@StringRes int stringId) {
        Context mCtx = Config.getConfig().getContext();
        return mCtx.getResources().getString(stringId);
    }
