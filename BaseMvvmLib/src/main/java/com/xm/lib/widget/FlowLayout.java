...
import com.xm.lib.widget.adapter.DataLoadAdapter;
...
public class FlowLayout<T> extends ViewGroup {
    ...
    private Class<? extends View> itemViewClz = null;
    ...
    public void setItemViewType(Class<? extends View> clz) {
        this.itemViewClz = clz;
    }

    public <V extends View> void setAdapter(List<T> data, DataLoadAdapter<V, T> adapter) {
        removeAllViews();
        if (null == itemViewClz) {
            throw new IllegalStateException("NEED TO SET THE TYPE OF ITEM VIEW");
        }
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            V view = (V) ReflectionUtils.createObject(itemViewClz, new Class[]{Context.class}, new Object[]{getContext()});
            view.setLayoutParams(layoutParams);
            if (null != adapter) {
                adapter.onLoading(view, t, i);
            }
            //往容器内添加View数据
            addView(view);
            view.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (null != onItemClickedListener) {
                        onItemClickedListener.onItemClicked(v, t);
                    }
                }
            });
        }
